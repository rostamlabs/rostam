...
func main() {
	...
	cacheKnobs := registerCacheKnobFlags(flag.CommandLine)
	...
	// The cache stanza both store paths share. -shards is added only on the
	// single-node path below: in -cluster mode it counts Raft shards, not cache shards.
	cacheCfg := rostam.CacheConfig{
		MaxMemoryBytes:        cacheMaxMemory,
		DisableColdCompaction: *disableColdCompaction,
		TTLSweepIntervalMs:    ttlSweepIntervalMs,
	}
	if err := cacheKnobs.apply(&cacheCfg, os.LookupEnv); err != nil {
		fatal(err.Error())
	}
	...
	*internalToken = resolveSecret("-internal-token", "ROSTAM_INTERNAL_TOKEN", *internalToken)
	...
	if *cluster {
		...
		cacheCfg.NumShardsPerNode = *shards
