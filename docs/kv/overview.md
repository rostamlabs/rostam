...
!!! warning "One key name is shadowed over REST, and only over REST"

    The keyless routes are literal paths under the same three-segment prefix the
    single-key routes use, so `ServeMux` prefers them over `{key}`. In practice
    that costs exactly one name: **`GET /v1/kv/indexes`** returns the index
    catalog, so a key literally named `indexes` can be written with `PUT` and
    deleted with `DELETE` but never read back over REST.

    Nothing else collides. The single-key routes are `GET`/`PUT`/`DELETE` only —
    there is no `POST /v1/kv/{key}` — so the `POST` routes (`query`, `indexes`,
    `flush`) shadow no key operation at all, and keys named `query` or `flush`
    stay fully readable and writable. `DELETE /v1/kv/indexes/{name}` is four
    segments, so it never competes with the three-segment `{key}` pattern either.

    This affects the HTTP surface only. The binary and gRPC transports address
    keys by value with no path grammar at all, so even `indexes` is fully
    readable and writable there — which is the remedy if you need that name.
