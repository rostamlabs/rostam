...
func TestInPlaceGuards(t *testing.T) {
	...
	t.Run("guard4b/stale-generation", func(t *testing.T) {
		...
		_, _, _, ok := s.inPlaceTargetLocked(key, []byte("bbbb"), hashKey(key))
	...
	t.Run("guard4c/hash-collision", func(t *testing.T) {
		...
		s.mu.Lock()
		_, _, _, ok := s.inPlaceTargetLocked(other, []byte("bbbb"), hashKey(victim))
		s.mu.Unlock()
		if ok {
		...
		s.mu.Lock()
		_, _, _, ok = s.inPlaceTargetLocked(victim, []byte("bbbb"), hashKey(victim))
		s.mu.Unlock()
		if !ok {
